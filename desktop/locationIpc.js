...
exports.setupLocationIpc = setupLocationIpc;
...
function setupLocationIpc() {
    ...
    (0, ipc_contract_1.handleIpc)("get-wifi-info", async () => {
        ...
        try {
            ...
            console.warn("[AINO] get-wifi-info failed:", err?.message);
            return {
                ok: false,
                error: err?.message || "wifi_lookup_failed",
            };
        }
    });
    ...
    (0, ipc_contract_1.handleIpc)("get-native-location", async () => {
        if (process.platform !== "win32") {
            return { ok: false, error: "unsupported_platform" };
        }
...
$w.Stop()
`.trim();
            const { stdout } = await execFileP("powershell.exe", ["-NoProfile", "-NonInteractive", "-Command", script], { timeout: 20000 });
            ...
            console.warn("[AINO] get-native-location failed:", err?.message);
            return {
                ok: false,
                error: err?.message || "native_location_failed",
            };
        }
    });
    ...
    (0, ipc_contract_1.handleIpc)("open-location-settings", async () => {
        try {
            if (process.platform === "win32") {
                await electron_1.shell.openExternal("ms-settings:privacy-location");
                return { ok: true };
            }
            if (process.platform === "darwin") {
                await electron_1.shell.openExternal("x-apple.systempreferences:com.apple.preference.security?Privacy_LocationServices");
                return { ok: true };
            }
            // Linux desktops don't have a unified privacy-location URI;
            // just open a help URL so the user can find the right toggle.
            await electron_1.shell.openExternal("https://www.google.com/search?q=enable+location+services+linux");
            return { ok: true };
        }
        catch (err) {
            console.warn("[AINO] open-location-settings failed:", err?.message);
            return { ok: false, error: err?.message || "open_failed" };
        }
    });
    (0, ipc_contract_1.handleIpc)("get-ip-location", async () => {
        ...
        for (const p of providers) {
            try {
                ...
                const resp = await electron_1.net.fetch(p.url, { method: "GET" });
                console.log(`[AINO] get-ip-location: ${p.url} responded ${resp.status}`);
                if (!resp.ok)
                    continue;
                const json = await resp.json();
                console.log(`[AINO] get-ip-location: ${p.url} body:`, JSON.stringify(json));
                const coords = p.parse(json);
                if (coords) {
                    console.log(`[AINO] IP geolocation via ${p.url} → ${coords.latitude},${coords.longitude}`);
                    return { ok: true, ...coords };
                }
                console.warn(`[AINO] get-ip-location: ${p.url} returned data but parse failed`);
            }
            catch (err) {
                console.warn(`[AINO] IP geolocation provider failed (${p.url}):`, err?.message);
            }
        }
...
//# sourceMappingURL=locationIpc.js.map