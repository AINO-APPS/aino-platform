...
import { createProxyRequest, normalizeProtocolPath, resolveClientFile, shouldApplyAppCsp } from "./protocolUtils";
...
electronSession.webRequest.onHeadersReceived((details, callback) => {
  const isAppDocument = shouldApplyAppCsp(details.url, details.resourceType);
...
protocol.handle("workpulse", async (request) => {
  ...
  if (pathname.startsWith("/api/") || pathname.startsWith("/api\\")) {
    try {
      ...
      const proxy = createProxyRequest(apiServer, request);
