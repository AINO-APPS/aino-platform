...
exports.setupProtocolHandling = setupProtocolHandling;
...
function setupProtocolHandling({ apiServer, clientDist, r2OriginPattern, electronSession }) {
    ...
    electron_1.protocol.handle("workpulse", async (request) => {
        const url = new URL(request.url);
        const pathname = (0, protocolUtils_1.normalizeProtocolPath)(url);
        ...
        if (pathname.startsWith("/uploads/") || pathname.startsWith("/uploads\\")) {
            ...
            try {
                ...
                if (range)
                    headers["range"] = range;
                let resp = await electron_1.net.fetch(`${apiServer}${pathname}${url.search || ""}`, {
                ...
                if (resp.status >= 300 && resp.status < 400 && location) {
                    ...
                    resp = await electron_1.net.fetch(location, {
                        method: request.method,
                    ...
                    followed = true;
                    resp = await electron_1.net.fetch(`${apiServer}${pathname}${url.search || ""}`, {
                        method: request.method,
                        headers,
                        credentials: "include",
                        redirect: "follow",
        ...
        if (pathname.startsWith("/api/") || pathname.startsWith("/api\\")) {
            ...
            try {
                ...
                const startedAt = Date.now();
                const resp = await electron_1.net.fetch(targetUrl, fetchOpts);
                console.log(`[proxy] ${request.method} ${targetUrl} -> ${resp.status} (${Date.now() - startedAt}ms)`);
                return resp;
            }
            catch (err) {
        ...
        const filePath = (0, protocolUtils_1.resolveClientFile)(clientDist, pathname);
        if (!filePath) {
        ...
        try {
            const data = await fs_1.default.promises.readFile(indexPath);
            return new Response(data, {
