 ...
 *
 * Usage:
 *   const { runTenantMigrations } = require('./platform/db/migrations');
 ...
 *
 * Resolved relative to this module so it works both from `server/platform/db/`
 * in tsx development and from the equivalent compiled `dist` directory.
 */
...
async function runMasterMigrations(): Promise<MigrationResult> {
    const { masterTransaction } = require("./pool");
